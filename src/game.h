...
class Game{
    public:
        ...
        void movePlayer(){
            ...
            // gameMap[player.getPosY()][player.getPosX()] = screen.BLANK_SPACE;

            player.setPosY(nextPos[0]);
            player.setPosX(nextPos[1]);
            // gameMap[player.getPosY()][player.getPosX()] = player.getAppearance();
        }
        ...
        void printEntities(){
            ...
            attron(COLOR_PAIR(3)); 
            mvprintw(player.getPosY(), player.getPosX()*2, "%c", (char)player.getAppearance());
